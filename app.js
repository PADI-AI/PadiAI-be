...
app.get('/completions', async (req, res) => {
  const options = {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        { role: "user", content: "If false, create a 4 week schedule on which topic to cover. One week can cover more than one topic and if more than 1 topic in a week, put it in array. Return only in JSON. Here's an example, \"week 1\": \"Variables and Data Types\"" }
      ],
      max_tokens: 200,
    })
  };
  ...
  try {
    ...
    if (data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content) {
      ...
      const strapiResponse = await fetch('http://localhost:1337/api/managers/1', strapiOptions);
